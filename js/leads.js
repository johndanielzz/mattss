  ...
  return `
    <article class="lead-card">
      ...
      <div class="lead-actions">
        ...
        <button class="btn btn-secondary" type="button" data-marketplace-list="${lead.id}" title="List on Marketplace">Sell</button>
        <button class="btn btn-secondary" type="button" data-campaign-add="${lead.id}" title="Add to Campaign">Campaign</button>
        ${renderWhatsappButton(lead, "btn btn-ghost")}
  ...
  return `
    <article class="lead-card local-lead-card">
      ...
      <div class="lead-actions">
        ...
        <button class="btn btn-secondary" type="button" data-marketplace-list="${escapeHtml(localLeadId(lead))}" title="List on Marketplace">Sell</button>
        <button class="btn btn-secondary" type="button" data-campaign-add="${escapeHtml(localLeadId(lead))}" title="Add to Campaign">Campaign</button>
        ${renderWhatsappButton(lead, "btn btn-ghost")}
        ${socialUrl ? `<a class="btn btn-ghost" href="${socialUrl}" target="_blank" rel="noreferrer">Social</a>` : ""}
        ${mapsUrl ? `<a class="btn btn-ghost" href="${mapsUrl}" target="_blank" rel="noreferrer">Maps</a>` : ""}
...
function initMarketplaceAndCampaignActions() {
  document.addEventListener("click", function(e) {
    var sellBtn = e.target.closest("[data-marketplace-list]");
    var campBtn = e.target.closest("[data-campaign-add]");
    if (!sellBtn && !campBtn) return;
    if (!requireAuth()) return;

    if (sellBtn) {
      var leadId = sellBtn.dataset.marketplaceList;
      var price = prompt("Enter price in cents (e.g. 499 = $4.99, 999 = $9.99):", "499");
      if (!price) return;
      var cents = parseInt(price, 10);
      if (isNaN(cents) || cents < 99) { alert("Minimum price is $0.99 (99 cents)."); return; }
      if (cents > 9999) { alert("Maximum price is $99.99 (9999 cents)."); return; }
      sellBtn.disabled = true;
      sellBtn.textContent = "Listing...";
      apiFetch("/api/marketplace/list", {
        method: "POST",
        body: JSON.stringify({ leadId: leadId, priceCents: cents })
      }).then(function(result) {
        sellBtn.textContent = "Listed!";
        var note = byId("searchNote");
        if (note) note.textContent = "Lead listed on Marketplace for $" + (cents / 100).toFixed(2) + ".";
      }).catch(function(err) {
        sellBtn.textContent = err.message;
      }).then(function() {
        setTimeout(function() { sellBtn.disabled = false; sellBtn.textContent = "Sell"; }, 2000);
      });
      return;
    }

    if (campBtn) {
      var leadId2 = campBtn.dataset.campaignAdd;
      // Redirect to campaigns page with lead pre-selected
      window.location.href = "/campaigns.html?addLead=" + encodeURIComponent(leadId2);
    }
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initMarketplaceAndCampaignActions();
});
